...
class NaoMock:
    def __init__(self, shared_memory, _map):
        ...
        self.advanced_movement_controller = AdvancedMovementController(
            self, shared_memory
        )
    ...
    def look_at(self, x_angle_degrees, y_angle_degrees):
        self.LOGGER.info(
            "New head position ({}, {})".format(x_angle_degrees, y_angle_degrees)
        )
    ...
    def get_goal(self):
        return (
            self.shared_memory.get_current_goal_position(),
            self.shared_memory.get_current_goal_direction(),
        )
